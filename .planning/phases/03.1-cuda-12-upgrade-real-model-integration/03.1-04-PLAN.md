...
Note: MASt3R camera estimation is bundled within TRELLIS-VGGT pipeline.
No separate MASt3R installation needed.
...
class ReconViaGenModel(BaseReconstructionModel):
    ...
    Note: Camera estimation (MASt3R/VGGT) is bundled within the pipeline.
    """
...
- Uses `postprocessing_utils.to_glb(gs, mesh)` instead of manual `_extract_mesh_data()`
- Removed brittle mesh attribute access (vertices, faces, etc.)
- Added GLB file size validation (>1KB for real geometry)
- Added GLB to OBJ/PLY conversion for compatibility
- Added comment clarifying MASt3R is bundled in TRELLIS-VGGT pipeline
  </action>
  <verify>
