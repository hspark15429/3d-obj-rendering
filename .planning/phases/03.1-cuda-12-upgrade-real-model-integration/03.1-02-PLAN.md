...
6. Clone TRELLIS code from microsoft repo:
```dockerfile
# Clone TRELLIS (microsoft official implementation)
RUN git clone --depth 1 https://github.com/microsoft/TRELLIS.git /app/trellis_src && \
    cp -r /app/trellis_src/trellis /app/ && \
    rm -rf /app/trellis_src
```

7. Set SPCONV_ALGO environment variable:
```dockerfile
...
Check: Build completes, all pip installs succeed
  ...
  <done>Dockerfile includes all TRELLIS dependencies (spconv, xformers, flash-attn, transformers)</done>
</task>
...
Wraps the microsoft/TRELLIS pipeline with our interface conventions.
Based on: https://huggingface.co/spaces/Stable-X/ReconViaGen/blob/main/app.py
"""
import os
import logging
...
# Set SPCONV_ALGO before importing trellis
...
- TRELLIS code cloned and available in container
